...
public:
	...
	constexpr Margin2_(const SimdData & simd) : _simd(simd) {}
	constexpr Margin2_(TagZero_T) : _simd(TagZero) {}
	constexpr Margin2_(T v) : _simd(SimdData::s_all(v)) {}
	...
	constexpr void set(T v)									{ _simd = SimdData(v,v,v,v); }
	constexpr void set(T x, T y)								{ _simd = SimdData(y,x,y,x); }
	constexpr void set(T top_, T right_, T bottom_, T left_)	{ _simd = SimdData(top_,right_,bottom_,left_); }

	AX_NODISCARD static constexpr This s_zero() { return SimdData::s_zero(); }
