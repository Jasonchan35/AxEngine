...
public:
	axExecuteString( const char* in, IString* out, IString* err ) {
		in_  = in;
...
axStatus ax_exec( const char* cmd, int& cmd_ret, const char*   std_in, IString*   std_out, IString*   std_err ) {
...
class axExecute_IOThread : public Thread {
public:
...
//					DEBUG_ax_log("stdin writing");
					...
					if( ! WriteFile( h, p->buf.ptr(), (DWORD)p->buf.size(), &dw, nullptr ) ) {
...
//					DEBUG_ax_log("thread stdin done");
				...
				case Node::t_stdout: {
					...
					assert( p->buf.size() != 0 );
					if( ! ReadFile( h, p->buf.ptr(), (DWORD)p->buf.size(), &dw, nullptr ) ) {
						if( dw == 0 ) {
...
static
axStatus	escpaceQuote( IString &o, const char* sz ) {
...
// Create the child process.     
   bSuccess = CreateProcess( nullptr,	//lpApplicationName 
