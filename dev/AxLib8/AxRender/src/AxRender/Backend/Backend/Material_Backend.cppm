...
public:
	...
	struct StructuredBufferParam : public ParamBase {
		...
		void setBufferPool(GpuBufferPool* pool) {
			AX_ASSERT(stride() <= pool->blockAlignment() || Math::isAlignedTo(stride(), pool->blockAlignment()));
	...
	bool setParam(NameId name, const i32x1&		v) { return _setVariable(name, v); }
	...
	bool setParam(NameId name, const u32x1&		v) { return _setVariable(name, v); }
	...
	bool setParam(NameId name, const f32x1&		v) { return _setVariable(name, v); }
	...
	bool setParam(NameId name, const f64x1&		v) { return _setVariable(name, v); }
