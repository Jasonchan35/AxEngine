...
bool axDX12RayTracing::create() {
...

	{
		...
		desc.type = RenderBufferType::RayTracingScratch;
		_scratchBuffer.create(desc);
	}

	{
		axRenderGpuBuffer_CreateDesc desc;
		desc.type = RenderBufferType::RayTracingInstanceDesc;
		_instanceDescsBuf.create(desc);
	}

	{
		axRenderGpuBuffer_CreateDesc desc;
		desc.type = RenderBufferType::RayTracingAccelStruct;
...
void axDX12RayTracing::_createRootSignature(ComPtr<ID3D12RootSignature>& outValue, const D3D12_ROOT_SIGNATURE_DESC& desc) {
	HRESULT hr;
...
void axDX12RayTracing::ShaderTable::create(axDX12RayTracing* rayTracing, Span<axStrLiteralW> functions, const RayGenParamData* paramData) {
	...
	for (auto& e : functions) {
		...
		auto view = ByteSpan(reinterpret_cast<axByte*>(p), shaderIdentifierSize);
	...
	_buf.create(RenderBufferType::ShaderRecord, data);
}

#endif // AX_RENDERER_DX12_DXR
