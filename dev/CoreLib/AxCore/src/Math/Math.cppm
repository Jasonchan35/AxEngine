...
template<class T> AX_INLINE void round_itself	( T & v ) { v = round(v); }
template<class T> AX_INLINE void floor_itself	( T & v ) { v = floor(v); }
template<class T> AX_INLINE void ceil_itself 	( T & v ) { v = ceil (v); }
...
AX_INLINE double rsqrt(double v) { return reciprocal(sqrt(v)); }
...
AX_NODISCARD AX_INLINE constexpr T smoothstep(const T& w) { return (-2.0f * w * w * w + 3.0f * w * w); }
...
template<class T> AX_NODISCARD AX_INLINE constexpr T easeIn (const T& a) { return a * a; }
template<class T> AX_NODISCARD AX_INLINE constexpr T easeOut(const T& a) { return 1 - a * a; }
