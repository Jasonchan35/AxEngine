...
export module AxCore.NumSIMD4;
export import AxCore.NumSIMD3;
...
public:
	...
	AX_NODISCARD AX_INLINE constexpr NumSIMD_(const T& e0, const T& e1, const T& e2, const T& e3) : Storage(e0, e1, e2, e3) {}
...
constexpr VEC NumSIMD_<4, VEC, STORAGE>::s_all(const T& v) {
...
constexpr bool NumSIMD_<4, VEC, STORAGE>::operator< (const VEC& v) const {
	if (!std::is_constant_evaluated()) {
		if constexpr (_use_SSE_f32) { return 0b1111 ==    _mm_movemask_ps(    _mm_cmp_ps(this->_m, v._m, _CMP_LT_OQ)); } 
		if constexpr (_use_SSE_f64) { return 0b1111 == _mm256_movemask_pd( _mm256_cmp_pd(this->_m, v._m, _CMP_LT_OQ)); } 
	}
	return _e0 < v._e0
		&& _e1 < v._e1
		&& _e2 < v._e2
...
constexpr bool NumSIMD_<4, VEC, STORAGE>::operator> (const VEC& v) const {
	if (!std::is_constant_evaluated()) {
		if constexpr (_use_SSE_f32) { return 0b1111 ==    _mm_movemask_ps(    _mm_cmp_ps(this->_m, v._m, _CMP_GT_OQ)); } 
		if constexpr (_use_SSE_f64) { return 0b1111 == _mm256_movemask_pd( _mm256_cmp_pd(this->_m, v._m, _CMP_GT_OQ)); } 
	}
	return _e0 > v._e0
		&& _e1 > v._e1
		&& _e2 > v._e2
...
constexpr bool NumSIMD_<4, VEC, STORAGE>::operator<=(const VEC& v) const {
	if (!std::is_constant_evaluated()) {
		if constexpr (_use_SSE_f32) { return 0b1111 ==    _mm_movemask_ps(    _mm_cmp_ps(this->_m, v._m, _CMP_LE_OQ)); } 
		if constexpr (_use_SSE_f64) { return 0b1111 == _mm256_movemask_pd( _mm256_cmp_pd(this->_m, v._m, _CMP_LE_OQ)); } 
	}
	return _e0 <= v._e0
		&& _e1 <= v._e1
		&& _e2 <= v._e2
...
constexpr bool NumSIMD_<4, VEC, STORAGE>::operator>=(const VEC& v) const {
	if (!std::is_constant_evaluated()) {
		if constexpr (_use_SSE_f32) { return 0b1111 ==    _mm_movemask_ps(    _mm_cmp_ps(this->_m, v._m, _CMP_GE_OQ)); } 
		if constexpr (_use_SSE_f64) { return 0b1111 == _mm256_movemask_pd( _mm256_cmp_pd(this->_m, v._m, _CMP_GE_OQ)); } 
	}
	return _e0 >= v._e0
		&& _e1 >= v._e1
		&& _e2 >= v._e2
...
template <class VEC, class STORAGE> AX_INLINE
constexpr VEC NumSIMD_<4, VEC, STORAGE>::abs() const {
