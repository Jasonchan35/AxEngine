...
void MaterialPass_Dx12::bind(ID3D12GraphicsCommandList* cmdList, DrawCall& drawCall) {
	auto* shaderPass_ = shaderPass();
	if (!shaderPass_) throw Error_Undefined();
	shaderPass_->bind(cmdList, drawCall);
	_bindParams(cmdList);
}

void MaterialPass_Dx12::bind(ID3D12GraphicsCommandList* cmdList, ComputeCall& computeCall) {
	auto* shaderPass_ = shaderPass();
	if (!shaderPass_) throw Error_Undefined();
...
void MaterialPass_Dx12::_bindParams(ID3D12GraphicsCommandList* cmdList) {
	auto* shaderPass_ = shaderPass();
	if (!shaderPass_) throw Error_Undefined();
	...
	{ //const buffer
		...
		for (auto& cb : _uniformBuffers) {
			auto* gpuBuf = rttiCastCheck<GpuBuffer_Dx12>(cb.currentGpuBuffer());
			...
			if (shaderPass_->isCompute()) {
				cmdList->SetComputeRootConstantBufferView(Dx12Util::castUINT(cbIndex), gpuBuf->gpuAddress());
			} else {
				cmdList->SetGraphicsRootConstantBufferView(Dx12Util::castUINT(cbIndex), gpuBuf->gpuAddress());
			}
			cbIndex++;
		}
	}
		...
		for (auto& texParam : _texParams) {
			...
			switch (tex->type()) {
				case RenderDataType::Texture2D: {
					auto* tex2d =  rttiCastCheck<Texture2D_Dx12>(tex);
					...
					desc.AddressU = Dx12Util::getDxSamplerWrap(ss.wrapU);
					desc.AddressV = Dx12Util::getDxSamplerWrap(ss.wrapV);
					desc.AddressW = Dx12Util::getDxSamplerWrap(ss.wrapW);
	...
	if (_storageBufParams.size() > 0 ) {
		...
		for (auto& sb : _storageBufParams) {
			auto* sbuf = rttiCastCheck<axDX12StorageBuffer>(sb.storageBuffer());
			if (!sbuf) throw Error_Undefined();
...
void MaterialPass_Dx12::onSetShaderPass(ShaderPass* shaderPass) {
}

void Material_Dx12::bind(ID3D12GraphicsCommandList* cmdList, DrawCall& drawCall) {
	auto* pass = rttiCastCheck<MaterialPass_Dx12>(_passes[drawCall.renderPass].ptr());
	pass->bind(cmdList, drawCall);
}

void Material_Dx12::bind(ID3D12GraphicsCommandList* cmdList, ComputeCall& computeCall) {
	auto* pass = rttiCastCheck<MaterialPass_Dx12>(_passes[computeCall.renderPass].ptr());
