...
class axDX12RayTracing : public axRenderRayTracing {
	AX_RTTI_INFO(axDX12RayTracing, axRenderRayTracing)
public:
	...
	class GlobalRootSignature : public NonCopyable {
	public:
		enum ParamId {
			OutputViewSlot,
		...
		ComPtr<ID3D12RootSignature>	_p;
	};

	class LocalRootSignature : public NonCopyable {
	public:
		enum ParamId {
	...
	class PipelineStateObject : public NonCopyable, axDX12CommonBase {
	public:
		PipelineStateObject();
		...
		void setLibShaderByteCode(ByteSpan byteCode) {
	...
	private:
		Int _addSubObject(D3D12_STATE_SUBOBJECT_TYPE type, const void* p) {
	...
	void _createRootSignature(ComPtr<ID3D12RootSignature>& outValue, const D3D12_ROOT_SIGNATURE_DESC& desc);
