...
: Base(desc)
{
	...
	{	// Create an root signature.
		...
		desc.NumParameters		= Dx12Util::castUINT(_psoRootParams.size());
		desc.pParameters		= _psoRootParams.data();
//		desc.NumStaticSamplers	= Dx12Util::castUINT(_psoRootStaticSamplers.size());
//		desc.pStaticSamplers	= _psoRootStaticSamplers.data();
		...
		Dx12Util::throwIfError(hr, error);

		auto* d3dDevice = Dx12Util::d3dDevice();
...
void ShaderPass_Dx12::onInit_DescriptorTable(DescriptorTable& descTable) {
	if (descTable._ranges.size() <= 0) return;

	descTable._rootParamIndex = Dx12Util::castUINT(_psoRootParams.size());
	...
	dst.DescriptorTable.NumDescriptorRanges = Dx12Util::castUINT(descTable._ranges.size());
	dst.DescriptorTable.pDescriptorRanges = descTable._ranges.data();
}

void ShaderPass_Dx12::onInitStageInfos(StageInfoSet& infoSet) {
	...
	for (auto& cb : _uniformBufferMap) {
		...
		dst.Descriptor.ShaderRegister = Dx12Util::castUINT(cb.bindPoint);
	...
	for (auto& tp : _texParams) {
		...
		dst.BaseShaderRegister	= Dx12Util::castUINT(p.bindPoint);
		dst.RegisterSpace		= 0;
	...
	for (auto& s : _samplerStateMap) {
		...
		dst.NumDescriptors		= 1;
		dst.BaseShaderRegister	= Dx12Util::castUINT(s.bindPoint);
		dst.RegisterSpace		= 0;
		dst.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
	}
	...
	for (auto& s : _samplerStateMap) {
		...
		dst.ShaderRegister = Dx12Util::castUINT(s.bindPoint);
		dst.RegisterSpace = 0;
	}
#endif
...
void ShaderPass_Dx12::bind(ID3D12GraphicsCommandList* cmdList, DrawCall& drawCall) {
	PsoKey key;
	...
	auto* pso = p->ptr();
	if (!pso) throw Error_Undefined();

	cmdList->SetGraphicsRootSignature(_rootSignature);
	cmdList->SetPipelineState(pso);
}

void ShaderPass_Dx12::bind(ID3D12GraphicsCommandList* cmdList, ComputeCall& computeCall) {
	cmdList->SetComputeRootSignature(_rootSignature);
	cmdList->SetPipelineState(_computePso);
}

void ShaderPass_Dx12::_createComputePipelineState(Pso& outPso) {
	auto* d3dDevice = Dx12Util::d3dDevice();
...
void ShaderPass_Dx12::_createPipelineState(Pso& outPso, const PsoKey& key) {
	auto* d3dDevice = Dx12Util::d3dDevice();
	...
	psoDesc.InputLayout.NumElements = Dx12Util::castUINT(vertexInputLayoutDesc.desc.size());
...
//-----
	...
	psoDesc.RasterizerState.CullMode = Dx12Util::getDxCullMode(_staticRenderState.cull);
	...
	psoDesc.DepthStencilState.DepthFunc			= Dx12Util::getDxDepthTestOp(_staticRenderState.depthTest.op);
...
//-----
	{	// blend
		...
		if (blend.isEnable()) {
			rt.BlendEnable	= true;
			rt.BlendOp			= Dx12Util::getDxBlendOp(blend.rgb.op);
			rt.BlendOpAlpha		= Dx12Util::getDxBlendOp(blend.alpha.op);
			rt.SrcBlend			= Dx12Util::getDxBlendFactor(blend.rgb.srcFactor);
			rt.DestBlend		= Dx12Util::getDxBlendFactor(blend.rgb.dstFactor);
			rt.SrcBlendAlpha	= Dx12Util::getDxBlendFactor(blend.alpha.srcFactor);
			rt.DestBlendAlpha	= Dx12Util::getDxBlendFactor(blend.alpha.dstFactor);
		}else{
...
ShaderPass_Dx12::VertexInputLayoutDesc::VertexInputLayoutDesc(const VertexAttrs& requiredVertexAttrs, const VertexDesc& vertexDesc) {
	for (auto& reqAttr : requiredVertexAttrs) {
		auto attrId = reqAttr.attrId;
		auto& attr = vertexDesc->attr(attrId);

		ShaderPass::validateVertexAttr(reqAttr, attr);
		...
		if (attrId >= VertexAttrId::TEXCOORD0 && attrId < VertexAttrId::TEXCOORD_END) {
			...
			dst.SemanticIndex = Dx12Util::castUINT(ax_enum_int(attrId - VertexAttrId::CUSTOM0) + axRenderVertexAttrId_kUvCount);
		...
		dst.Format = Dx12Util::getDxDataType(attr.dataType);
	}
}
